...
def get_efficiency_label(price_response):
    if price_response <= -0.5: # Colapso de eficiencia (setup excelente para short)
        return "🔻" #"💥"
    ...
    if not results:
        ...
        for item in results:
            ...
            line1 = (
                ...
                #f"EFFI: {item['efficiency']:>6.3f}  "
                f"EFFI({get_efficiency_label(item['efficiency'])}): {item['efficiency']:>6.3f}  "
